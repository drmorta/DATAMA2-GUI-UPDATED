	...
	<div class="card-body">
        ...
        <div class="container-fluid">
			<table class="table table-bordered table-stripped">
				...
				<tbody>
					<?php 
						$qry = $conn->query("SELECT * from `customer` order by (`Cust_ID`) asc ");
						while($row = $qry->fetch_assoc()):
					?>
						<tr>
							<td><?php echo ucwords($row['Cust_ID']) ?></td>
							<td><?php echo ucwords($row['Cust_Name']) ?></td>
							<td><?php echo ucwords($row['Cust_UN']) ?></td>
							<td ><p class="m-0 truncate-1"><?php echo $row['Cust_Email'] ?></p></td>
							<td ali	gn="center">
